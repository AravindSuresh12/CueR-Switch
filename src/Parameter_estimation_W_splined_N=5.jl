...
for trial_index = 1:number_of_trials
   ...
   # do a local step -
    if (mod(trial_index,2) == 0)
        ...
        # local refine -
        pV = local_refienment_step(path_to_data_dir, pV_best; iteration_max=5)
    end

    # main -
    (EC,PC,RA) = main(path_to_data_dir, vec(pV); rank_cutoff=4,maximum_number_of_iterations=5)

    # dump results to disk -
    local  fname = "./simulated/POETS_N_Vary/N=5/RA_T$(trial_index).dat"
    writedlm(fname,RA)
    fname = "./simulated/POETS_N_Vary/N=5/EC_T$(trial_index).dat"
    writedlm(fname,EC)
    fname = "./simulated/POETS_N_Vary/N=5/PC_T$(trial_index).dat"
    writedlm(fname,PC)

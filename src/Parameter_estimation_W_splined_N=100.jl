...
pV =pvec_initial #neighbor_function(pvec_initial; sigma=0.25) #No Perturbation for N=100 and N=1000 runs for temperature iterations- 
EC = 0
PC = 0
RA = 0
...
for trial_index = 1:number_of_trials
   ...
   # do a local step -
    if (mod(trial_index,2) == 0)
        ...
        # local refine -
        pV = local_refienment_step(path_to_data_dir, pV_best; iteration_max=100)
    end

    # main -
    (EC,PC,RA) = main(path_to_data_dir, vec(pV); rank_cutoff=4,maximum_number_of_iterations=100)

    # dump results to disk -
    local  fname = "./simulated/POETS_N_Vary/N=100/RA_T$(trial_index).dat"
    writedlm(fname,RA)
    fname = "./simulated/POETS_N_Vary/N=100/EC_T$(trial_index).dat"
    writedlm(fname,EC)
    fname = "./simulated/POETS_N_Vary/N=100/PC_T$(trial_index).dat"
    writedlm(fname,PC)
